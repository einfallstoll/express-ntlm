...
module.exports = function(options) {
    ...
    function getCookieByKey(request, key) {
        ...
        return cookie.split(';').reduce(function(result, element) {
    ...
    return function(request, response, next) {
        ...
        var cookieNtlmSessionId = getCookieByKey(request, options.cookieName);

        if (cookieNtlmSessionId && ntlm_version !== 1) {
            request.connection.id = cookieNtlmSessionId;
        }
        if (!request.connection.id) {
            request.connection.id = options.generateCookie();
            response.cookie(options.cookieName, request.connection.id)
        }
